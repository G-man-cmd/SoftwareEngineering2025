...
from player_actions import build_player_action_screen
...
def on_tab(event: tk.Event, root: tk.Tk, entry_ids: Dict, users: Dict, builder: pygubu.Builder, db: Database) -> None:
    ...
    if "equipment_id" in entry_field_id:
        ...
        if equipment_id in [user.equipment_id for user in users["blue"]] or equipment_id in [user.equipment_id for user
                                                                                             in users["red"]]:
    ...
    # If the entry field ID is a user ID field, query database for user with matching ID
        ...
        if matching_players:
            ...
            if user_id in [user.user_id for user in users["blue"]] or user_id in [user.user_id for user in
                                                                                  users["red"]]:
            ...
            root.after_idle(lambda: users["blue"].append(
                Player(int(entry_field_id.split("_")[-1]), equipment_id, user_id, codename,
                     "blue")) if "blue" in entry_field_id else users["red"].append(
                Player(int(entry_field_id.split("_")[-1]), equipment_id, user_id, codename, "red")))
            ...
            if row != 15:
                next_entry_field = builder.get_object(
                    entry_field_id.replace(f"user_id_{row}", f"equipment_id_{row + 1}"), root)
    ...
    # If the user tabs from the codename entry field, insert the user into the database if they don't already exist
        ...
        if codename in [user.codename for user in users["blue"]] or codename in [user.codename for user in
                                                                                 users["red"]]:
        ...
        users["blue" if "blue" in entry_field_id else "red"].append(
            Player(int(entry_field_id.split("_")[-1]), equipment_id, user_id, codename,
                 "blue" if "blue" in entry_field_id else "red"))
...
def on_f5(main_frame: tk.Tk, root: tk.Tk, users: Dict, network: Network, db: Database, builder: pygubu.Builder,
          entry_ids: Dict) -> None:
...
def build(root: tk.Tk, users: Dict[str, List[Player]], network: Network, db: Database) -> None:
    # Load the UI file and create the builder
    builder: pygubu.Builder = pygubu.Builder()
    builder.add_from_file("assets/backgrounds/player_entries.ui")
    ...
    for i in range(1, 16):
        for field in fields:
            entry_ids[builder.get_object(f"{field}{i}",
    ...
    builder.get_object("submit", main_frame).configure(
        command=lambda: on_f5(main_frame, root, users, network, db, builder, entry_ids))
    builder.get_object("clear", main_frame).configure(command=lambda: on_f12(main_frame, entry_ids, users, builder, db))


def rebind_events(builder: pygubu.Builder, main_frame: tk.Tk, root: tk.Tk, users: Dict, network: Network,
                  entry_ids: Dict, db: Database) -> None:
...
def return_to_entry_screen(root: tk.Tk, action_screen, main_frame: tk.Frame, builder: pygubu.Builder, entry_ids: Dict,
                           users: Dict, db: Database) -> None:
